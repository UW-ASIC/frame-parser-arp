...
module frame_parser_header_datapath (
    ...
    // payload_valid/payload_bit_offset must stay combinationally aligned
    // with out_tdata -- frame_parser_payload_aligner reads out_tdata
    // "live" (via w_tdata), not a registered copy, so these can't be
    // clocked like dst_mac/src_mac/final_ethertype below or they'd
    // describe a beat that's already one cycle stale by the time anything
    // reads them (the aligner would silently combine the wrong pair of
    // beats, dropping the first bytes of payload).
    always @(*) begin
        payload_valid = 1'b0;
        ...
        if (out_tvalid && tready) begin
            if (en_dst_mac) begin
                payload_valid = 1'b0;
            end else if (en_src_mac_part1) begin
                payload_valid = 1'b0;
            end else if (en_src_mac_part2) begin
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (out_tvalid && tready) begin // tready is currently tied high by frame_control, so this gate is a no-op today -- it's here so this block stays correct if that ever changes.
                if (en_dst_mac) begin
                    dst_mac <= {
                        out_tdata[7:0],
                        out_tdata[15:8],
