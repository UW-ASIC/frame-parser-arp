...
module frame_parser_frame_control (
  ...
  always@(*) begin
    case (tkeep) 
      8'b00000001 : bytes_kept = 5'd1;
      8'b00000011 : bytes_kept = 5'd2;
      8'b00000111 : bytes_kept = 5'd3;
      8'b00001111 : bytes_kept = 5'd4;
      8'b00011111 : bytes_kept = 5'd5;
      8'b00111111 : bytes_kept = 5'd6;
      8'b01111111 : bytes_kept = 5'd7;
      8'b11111111 : bytes_kept = 5'd8;
      default     : bytes_kept = 5'd0;
    endcase
  end
  ...
  always@(posedge clk, negedge rst_n) begin
    if (!rst_n) begin
      ...
      if ((next_state == BEAT_1)) begin
        frame_len <= bytes_kept;
  ...
  always@(*) begin
    if (tvalid) begin
      case (present_state) 
        ...
        BEAT_2      : begin
          // Use the registered vlan_remembered, NOT the combinational
          // is_vlan_frame. is_vlan_frame depends on vlan_detected, which
          // depends on next_state -- reading it here closes a combinational
          // loop (next_state -> vlan_detected -> is_vlan_frame -> next_state).
          // Icarus resolves that iteratively, but it is a real cycle to
          // synthesis and has no valid timing path on FPGA/ASIC.
          //
          // Behaviour is unchanged: this branch only runs when
          // present_state == BEAT_2, and vlan_detected is qualified by
          // (next_state == BEAT_2), which is false here -- so is_vlan_frame
          // already reduced to vlan_remembered. vlan_remembered was latched
          // on the edge where next_state == BEAT_2 (i.e. during BEAT_1), so
          // it holds the correct detection result by the time we read it.
          if (vlan_remembered) begin
  ...
  always@(*) begin
    ...
    if (tlast && tvalid) begin
      ...
      final_frame_len = frame_len + bytes_kept;
    end else begin
      drop = 1'b0;
