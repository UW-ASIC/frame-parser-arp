...
module arp_responder_reply_beat_mux #(
    ...
    */
    ...
    // Padded: every one of the 8 beats is fully valid -- beats 6-7 and the
    // tail of beat 5 carry real (zero) pad bytes that must be transmitted.
    // Unpadded: the frame ends mid-beat-5, so only its first 2 bytes count.
    assign tkeep = PAD_TO_MIN ? ((beat_idx <= 3'd7) ? 8'hFF : 8'h00)
                              : ((beat_idx <= 3'd4) ? 8'hFF :
                                 (beat_idx == 3'd5) ? 8'h03 : 8'h00);
