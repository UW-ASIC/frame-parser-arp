...
class Design:
    ...
    async def get_tkeep(self, beat_number: int) -> LogicArray:
        """
        Manually get tkeep for a beat by beat number
        """
        # Set beat number in design
        self._dut.beat_idx.value = beat_number
        # Wait for 1 clock cycle
...
@cocotb.test
async def tkeep_values(dut):
    """
    Tests tkeep under the default PAD_TO_MIN=1: the reply is padded out to
    the 64-byte Ethernet minimum, so all 8 beats are fully valid -- the pad
    bytes after the 42 real ones are still bytes that must be transmitted.
    (Unpadded, beat 5 would be 0x03 and beats 6-7 would be 0x00.)
    """
    # Get logger
    logger = logging.getLogger("tkeep_values")
    # Initialize design
    logger.debug("Initializing design")
    design = Design(dut, logger)
    await design.reset()
    ...
    # All 8 beats of a padded reply are fully valid
    for i in range(8):
        tkeep = await design.get_tkeep(i)
        assert tkeep == 0xFF, f"beat {i}: expected tkeep=0xFF, got {tkeep}"
