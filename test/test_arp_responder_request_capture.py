...
async def test_basic_capture(dut):
    ...
    # build_arp_request always emits the standard Ethernet/IPv4 ARP header
    # constants (see HTYPE/PTYPE/HLEN/PLEN in build_arp_request above).
    assert int(dut.ext_htype.value) == 0x0001, \
        f"HTYPE mismatch: got {int(dut.ext_htype.value):#06x}, expected 0x0001"

    assert int(dut.ext_ptype.value) == 0x0800, \
        f"PTYPE mismatch: got {int(dut.ext_ptype.value):#06x}, expected 0x0800"

    assert int(dut.ext_hlen.value) == 0x06, \
        f"HLEN mismatch: got {int(dut.ext_hlen.value):#04x}, expected 0x06"

    assert int(dut.ext_plen.value) == 0x04, \
        f"PLEN mismatch: got {int(dut.ext_plen.value):#04x}, expected 0x04"
