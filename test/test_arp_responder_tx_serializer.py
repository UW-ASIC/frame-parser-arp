...
# arp_responder_tx_serializer defaults to PAD_TO_MIN=1, so a reply is padded
# out to the 64-byte Ethernet minimum: 8 beats, last beat index 7.
LAST_BEAT = 7
N_BEATS = LAST_BEAT + 1
...
async def test_stall_midframe(dut):
    ...
    for expected_beat in range(3, N_BEATS):
        await FallingEdge(dut.clk)
        assert dut.m_tvalid.value == 1
        assert dut.beat_idx.value == expected_beat
        if expected_beat == LAST_BEAT:
...
async def test_stall_at_start(dut):
    ...
    for expected_beat in range(1, N_BEATS):
        await FallingEdge(dut.clk)
        assert dut.m_tvalid.value == 1
        assert dut.beat_idx.value == expected_beat
        if expected_beat == LAST_BEAT:
...
async def test_stall_on_last_beat(dut):
    ...
    for expected_beat in range(LAST_BEAT):
        await FallingEdge(dut.clk)
        assert dut.m_tvalid.value == 1
        assert dut.beat_idx.value == expected_beat
        await RisingEdge(dut.clk)
    ...
    await FallingEdge(dut.clk)
    assert dut.m_tvalid.value == 1
    assert dut.beat_idx.value == LAST_BEAT
    assert dut.m_tlast.value == 1
    dut.m_tready.value = 0
...
async def test_random_backpressure(dut):
    ...
    for _ in range(100):
        ...
        if valid_before == 1 and ready_before == 1:
            ...
            if beat_before == LAST_BEAT:
                assert last_before == 1
            else:
                assert last_before == 0

        if prev_valid == 1 and valid_after == 0:
            assert handshake_beats == list(range(N_BEATS))
        prev_valid = valid_after

        if handshake_beats == list(range(N_BEATS)):
...
async def test_fire_held_high(dut):
    ...
    for expected_beat in range(N_BEATS): # 0..LAST_BEAT
...
async def test_data_passthrough(dut):
    ...
    for expected_beat in range(N_BEATS):
        ...
        keep = 0xFF  # every beat of a padded reply is fully valid
...
async def test_soak(dut):
    ...
    for frame in range(10):
        ...
        # Random backpressure until every beat is delivered. Sampling on the
        handshakes = []
        cycles = 0
        while len(handshakes) < N_BEATS:
            cycles += 1
            assert cycles < 100, f"frame {frame}: timed out waiting for {N_BEATS} handshakes"
            await FallingEdge(dut.clk)
        ...
        assert handshakes == list(range(N_BEATS)), f"frame {frame}: handshake beats were {handshakes}"
